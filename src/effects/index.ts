...
export const EFFECTS: EffectDef[] = [
  ...
  },
  {
    ...
    params: [
      ...
      { type: 'slider', id: 'suck', label: 'Fly Through', min: -1, max: 1, step: 0.01, default: 0.5, surprise: [-0.9, 0.9], integrate: true },
  ...
  },
  {
    ...
    params: [
      ...
      { type: 'slider', id: 'ffly', label: 'Speed', min: 0, max: 2, step: 0.01, default: 0.55, surprise: [0.2, 1.2], integrate: true },
      ...
      { type: 'slider', id: 'fspin', label: 'Spin', min: -1, max: 1, step: 0.01, default: 0, surprise: [-0.5, 0.5], integrate: true },
      { type: 'slider', id: 'fwander', label: 'Camera Wander', min: 0, max: 0.35, step: 0.005, default: 0.2, surprise: [0.05, 0.32] },
    ],
  },
  {
    id: 'nebula',
    ...
    params: [
      ...
      { type: 'slider', id: 'ndetail', label: 'Detail', min: 2, max: 5, step: 1, default: 3, perfScale: true },
  ...
  },
  {
    ...
    params: [
      ...
      { type: 'slider', id: 'grot', label: 'Rotation', min: -1, max: 1, step: 0.01, default: 0.15, surprise: [-0.6, 0.6], integrate: true },
  ...
  },
  {
    ...
    params: [
      ...
      { type: 'slider', id: 'bspeed', label: 'Camera Speed', min: 0, max: 2, step: 0.01, default: 0.6, surprise: [0.2, 1.3], integrate: true },
  ...
  },
  {
    ...
    params: [
      ...
      { type: 'slider', id: 'mzoom', label: 'Endless Zoom', min: 0, max: 2, step: 0.01, default: 0.8, surprise: [0.3, 1.5], integrate: true },
  ...
  },
  {
    ...
    params: [
      ...
      { type: 'slider', id: 'aspin', label: 'Spin', min: -1, max: 1, step: 0.01, default: 0.3, surprise: [-0.8, 0.8], integrate: true },
  ...
  },
  {
    ...
    params: [
      ...
      { type: 'slider', id: 'fspin', label: 'Spin', min: -1, max: 1, step: 0.01, default: 0.15, surprise: [-0.7, 0.7], integrate: true },
...
/**
 * Every parameter becomes `uniform float u_<id>`, so an id that matches a name the
 * prelude already declares makes the shader fail to compile with a bare
 * "redefinition" error — and the scene silently stops rendering. That shipped once
 * (u_detail, which Mandala and Liquid Marble both own); this makes it impossible
 * to miss again.
 */
...
for (const e of EFFECTS) {
  for (const p of e.params) {
    if (RESERVED_UNIFORMS.has(p.id)) {
      throw new Error(
        `[dreamloop] effect "${e.id}" has a parameter named "${p.id}", which collides with the ` +
          `engine uniform u_${p.id}. Rename the parameter.`,
      );
    }
  }
  ...
  for (const p of e.params) {
    if (seen.has(p.id)) {
      throw new Error(`[dreamloop] effect "${e.id}" declares parameter "${p.id}" twice.`);
    }
    seen.add(p.id);
  }
